...
class KnapsackScheduler:
    def __init__(self):
        ...
        self.state_size = self.num_items + self.num_knapsacks
        self.actions = [(item, knapsack) for item in self.items for knapsack in range(self.num_knapsacks)]
        self.reset()

    def reset(self):
        self.state = [0] * self.num_items + [0] * self.num_knapsacks
        self.total_value = 0
    ...
    def step(self, action):
        ...
        if next_state[item_index] < len(WEIGHT[item]):
            ...
            next_state[self.num_items + knapsack] += WEIGHT[item][next_state[item_index] - 1]
        ...
        done = all(self.state[self.num_items + i] > CAPA[i] for i in range(self.num_knapsacks)) or \
               all(self.state[i] == len(WEIGHT[self.items[i]]) for i in range(self.num_items))
    ...
    def is_valid_action(self, action):
        ...
        if self.state[item_index] < len(WEIGHT[item]):
            new_weight = self.state[self.num_items + knapsack] + WEIGHT[item][self.state[item_index]]
            return new_weight <= CAPA[knapsack]
        return False
...
def train(n_episodes=N_EPISODES):
    ...
    for episode in range(n_episodes):
        ...
        if episode % (N_EPISODES//10) == 0:
            print(f"Episode: {episode}, Selected Items: {env.selected_items}, Total Value: {env.total_value}, Knapsack Weights: {env.state[env.num_items:]}, Epsilon: {agent.epsilon:.4f}")
...
def test(env, agent):
    ...
    print("Knapsack Weights:", env.state[env.num_items:])
